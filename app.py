...
with mysql.connector.connect(host="localhost",user="root",password="system",database="flaskblog"):
    ...
    cursor.execute("create table if not exists registration(Username varchar(40) primary key,Mobile varchar(60) unique,Address varchar(60),Password varchar(50))")
...
def register():
    if request.method=="POST":
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute("insert into registration values(%s,%s,%s,%s)",[Username,Mobile,Address,Password])
        mydb.commit()
        cursor.close()
